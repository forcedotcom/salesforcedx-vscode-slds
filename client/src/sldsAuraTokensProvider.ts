...
function shouldTriggerCompletions(document: vscode.TextDocument, position: vscode.Position): vscode.Range | undefined {
  ...
  const triggerRange = document.getWordRangeAtPosition(position, /\bt\([a-z\d]*\)?/i);
  return triggerRange;
}
...
function getCompletions(range: vscode.Range): vscode.CompletionList {
  const completions = Object.values(tokens)
    .filter(token => token.deprecated !== 'true')
    .map(token => {
      const completionItem = new vscode.CompletionItem(token.auraToken);
      const text = 't(' + token.auraToken + ')';
...
export function register(context: vscode.ExtensionContext): vscode.Disposable {
  ...
  const provider = {
    provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext) {
      const triggerRange = SLDSContext.isEnable(extensionContext, ContextKey.GLOBAL, ContextKey.AUTO_SUGGEST, ContextKey.DESIGN_TOKEN)
