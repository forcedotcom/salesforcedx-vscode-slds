...
function shouldTriggerCompletions(document: vscode.TextDocument, position: vscode.Position): vscode.Range | undefined {
  ...
  const triggerRange = document.getWordRangeAtPosition(position, /\bslds-[\w-]*/i);
  return triggerRange;
}
...
function getCompletions(range: vscode.Range): vscode.CompletionList {
  const completions = utilities.classes.map(cssClass => {
    const completionItem = new vscode.CompletionItem(cssClass.selector);
